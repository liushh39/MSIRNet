...
class PairedImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.mask_grid1_paths = get_child_dirs(self.mask_grid1_folder)
        self.mask_grid2_paths = get_child_dirs(self.mask_grid2_folder)
        self.mask_grid3_paths = get_child_dirs(self.mask_grid3_folder)
    ...
    def __getitem__(self, index):
        ...
        ######__________随机读取masks(LOL使用，RELLISUR不使用)#######
        # mask_grid1_path = self.mask_grid1_paths[index]
        # img_mask_grid11 = []
        # for mask in sorted(glob.glob(os.path.join(mask_grid1_path, '*'))):
        #     img_mask_grid1 = cv2.imread(mask).astype(np.float32) / 255.
        #     img_mask_grid11.append(img_mask_grid1)
        #
        # mask_grid2_path = self.mask_grid2_paths[index]
        # img_mask_grid22 = []
        # for mask in sorted(glob.glob(os.path.join(mask_grid2_path, '*'))):
        #     img_mask_grid2 = cv2.imread(mask).astype(np.float32) / 255.
        #     img_mask_grid22.append(img_mask_grid2)
        #
        # mask_grid3_path = self.mask_grid3_paths[index]
        # img_mask_grid33 = []
        # for mask in sorted(glob.glob(os.path.join(mask_grid3_path, '*'))):
        #     img_mask_grid3 = cv2.imread(mask).astype(np.float32) / 255.
        #     img_mask_grid33.append(img_mask_grid3)

        ######__________按顺序读取masks__________#######
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.opt['use_resize_crop']:
                ...
                ######__________随机读取masks，并且random_resize(RELLISUR使用，LOL不使用)#######
                mask_grid1_path = self.mask_grid1_paths[index]
                img_mask_grid11 = []
                for mask in sorted(glob.glob(os.path.join(mask_grid1_path, '*'))):
                    img_mask_grid1 = cv2.imread(mask).astype(np.float32) / 255.
                    img_mask_grid1 = random_resize(img_mask_grid1, resize_factor)
                    img_mask_grid11.append(img_mask_grid1)
                ...
                for mask in sorted(glob.glob(os.path.join(mask_grid2_path, '*'))):
                    img_mask_grid2 = cv2.imread(mask).astype(np.float32) / 255.
                    img_mask_grid2 = random_resize(img_mask_grid2, resize_factor)
                    img_mask_grid22.append(img_mask_grid2)
                ...
                for mask in sorted(glob.glob(os.path.join(mask_grid3_path, '*'))):
                    img_mask_grid3 = cv2.imread(mask).astype(np.float32) / 255.
                    img_mask_grid3 = random_resize(img_mask_grid3, resize_factor)
                    img_mask_grid33.append(img_mask_grid3)
        ...
        img_mask_grid111 = torch.cat(
            [img2tensor(img_mask, bgr2rgb=True, float32=True) for img_mask in img_mask_grid11], dim=0)
        img_mask_grid222 = torch.cat(
            [img2tensor(img_mask, bgr2rgb=True, float32=True) for img_mask in img_mask_grid22], dim=0)
        img_mask_grid333 = torch.cat(
            [img2tensor(img_mask, bgr2rgb=True, float32=True) for img_mask in img_mask_grid33], dim=0)
