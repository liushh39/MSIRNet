...
from basicsr.archs.lesnet.semantic_embedding import TransformerBlock, TransformerBlock_modified
from basicsr.archs.lesnet.hybrid_parallel_attention import OSA_Block, OSA_Block_modified, OSA_Block_hybrid_modified_s
from basicsr.archs.lesnet.fncm import FRNet
...
class LESNet17(nn.Module):
    def __init__(self,
        ...
        if self.scale_factor == 4:
            self.conv_first_1 = nn.Conv2d(3, ch4, 3, 2, 1, bias=True)  # X4
        elif self.scale_factor == 2:
            self.conv_first_1 = nn.Conv2d(3, ch4, 3, 4, 1, bias=True)  # X2

        self.resblock1 = ResBlock(ch4, ch4, norm_type, act_type)
        self.resblock2 = ResBlock(ch4, ch4, norm_type, act_type)
        self.up = nn.Upsample(scale_factor=2)
        self.conv_first_2 = nn.Conv2d(ch4, ch3, 3, stride=1, padding=1)
        self.conv_first_3 = nn.Conv2d(ch3, ch2, 3, stride=1, padding=1)

        ###__SA-EB:Semantic-Aware Embedding Block
        self.SA_EB_1 = TransformerBlock_modified(256, 256)
        self.SA_EB_2 = TransformerBlock_modified(128, 128)
        self.SA_EB_3 = TransformerBlock_modified(64, 64)

        ###__DP-BAB:Dual-Path Blend Attention Block
        self.DP_BAB1 = OSA_Block_hybrid_modified_s(channel_num=ch4, window_size=8, with_pe=True) ###__16,8,4 for check_image_size 16
        self.DP_BAB2 = OSA_Block_hybrid_modified_s(channel_num=ch3, window_size=8, with_pe=True)
        self.DP_BAB3 = OSA_Block_hybrid_modified_s(channel_num=ch2, window_size=8, with_pe=True)

        self.conv_last1 = nn.Conv2d(ch4 + ch4, ch4, 1)
        self.conv_last2 = nn.Conv2d(ch3 + ch3, ch3, 1)
        self.conv_last3 = nn.Conv2d(ch2 + ch2, ch2, 1)
        self.conv_last4 = nn.Conv2d(16, 3, 3, 1, 1)
        self.upsample1 = pixelshuffle_block(ch4, ch3, 2, bias=True)
        self.upsample2 = pixelshuffle_block(ch3, ch2, 2, bias=True)
        self.upsample3 = pixelshuffle_block(ch2, 16, 2, bias=True)
        self.fncm1 = FRNet(ch4)
        self.fncm2 = FRNet(ch3)
        self.fncm3 = FRNet(ch2)

    def encode_and_decode(self, input, mask1, mask2, mask3, gt_indices=None, current_iter=None):

        enc_feats = []
        L1 = self.conv_first_1(input.detach())
        L1 = self.resblock1(L1)
        L1 = self.resblock2(L1)

        M1_1 = self.SA_EB_1(L1, mask3)
        M1_2 = self.DP_BAB1(M1_1)
        enc_feats.append(M1_2)  #256,32,32

        L2 = self.up(M1_2)
        L2 = self.conv_first_2(L2)
        M2_1 = self.SA_EB_2(L2, mask2)
        M2_2 = self.DP_BAB2(M2_1)  #128,64,64
        enc_feats.append(M2_2)

        L3 = self.up(M2_2)
        L3 = self.conv_first_3(L3)
        M3_1 = self.SA_EB_3(L3, mask1)
        M3_2 = self.DP_BAB3(M3_1)   #64,128,128
        enc_feats.append(M3_2)
        ...
        for i in range(self.max_depth):
            ...
            if cur_res in self.codebook_scale:  # needs to perform quantize
                if prev_dec_feat is not None:   # below operations are Residual Shortcut Module
        ...
        M4_1 = self.conv_last1(torch.cat([M1_2, x.detach()], 1))  # x.detach() no_grad
        M4_1 = self.fncm1(M4_1)
        M4_2 = self.upsample1(M4_1)
        M4_3 = self.conv_last2(torch.cat([M2_2, M4_2], 1))
        M4_3 = self.fncm2(M4_3)
        M4_4 = self.upsample2(M4_3)
        M4_5 = self.conv_last3(torch.cat([M3_2, M4_4], 1))
        M4_5 = self.fncm3(M4_5)
        M3_6 = self.upsample3(M4_5)
        out_img = self.conv_last4(M3_6)   # upscale 2/4
    ...
    @torch.no_grad()
    def test_tile(self, input, mask1, mask2, mask3, tile_size=240, tile_pad=16):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                mask1_tile = mask1[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]
                mask2_tile = mask2[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]
                mask3_tile = mask3[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]

                # upscale tile
                output_tile = self.test(input_tile, mask1_tile, mask2_tile, mask3_tile)
    ...
    @torch.no_grad()
    def test(self, input, mask1, mask2, mask3):
        ...
        mask1 = torch.cat([mask1, torch.flip(mask1, [2])], 2)[:, :, :h_old + h_pad, :]
        mask1 = torch.cat([mask1, torch.flip(mask1, [3])], 3)[:, :, :, :w_old + w_pad]
        mask2 = torch.cat([mask2, torch.flip(mask2, [2])], 2)[:, :, :h_old + h_pad, :]
        mask2 = torch.cat([mask2, torch.flip(mask2, [3])], 3)[:, :, :, :w_old + w_pad]
        mask3 = torch.cat([mask3, torch.flip(mask3, [2])], 2)[:, :, :h_old + h_pad, :]
        mask3 = torch.cat([mask3, torch.flip(mask3, [3])], 3)[:, :, :, :w_old + w_pad]


        dec, _, _, _ = self.encode_and_decode(input, mask1, mask2, mask3)
    ...
    def forward(self, input, mask1, mask2, mask3, gt_indices=None):

        if gt_indices is not None:
            # in LQ training stage, need to pass GT indices for supervise.
            dec, codebook_loss, semantic_loss, indices = self.encode_and_decode(input, mask1, mask2, mask3, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            dec, codebook_loss, semantic_loss, indices = self.encode_and_decode(input, mask1, mask2, mask3)
