...
# window spatial attention related classes(channel), modified the dropout progress
class hybrid_Attention_LS_SA(nn.Module):
    def __init__(
            self,
            dim,
            dim_head=32,
    ...
    def forward(self, x):
        ...
        # combine heads out
        out = self.to_out(out)
        output = rearrange(out, '(b x y) ... -> b x y ...', x=height, y=width)

        return output


class OSA_Block_hybrid_modified_s_local_spatial_SA(nn.Module):
    def __init__(self, channel_num=64, bias=True, ffn_bias=True, window_size=8, with_pe=False, dropout=0.0):
        super(OSA_Block_hybrid_modified_s_local_spatial_SA, self).__init__()
        ...
        self.layer = nn.Sequential(
            ...
            Rearrange('b d (x w1) (y w2) -> b x y w1 w2 d', w1=w, w2=w),  # 对输入进行维度重排 b:B,d:C,w:W,
            PreNormResidual(channel_num,
                            hybrid_Attention_LS_SA(dim=channel_num, dim_head=channel_num // 4, dropout=dropout,
                                                      window_size=window_size, with_pe=with_pe)),  # spatial-attention
            Rearrange('b x y w1 w2 d -> b d (x w1) (y w2)'),
            Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim=channel_num, dropout=dropout)),
            # FeedForward network
...
# global spatial attention related classes(channel), modified the dropout progress
class hybrid_Attention_GS_SA(nn.Module):
    def __init__(
            self,
            dim,
            dim_head=32,
        ...
        self.attn_drop = nn.Dropout(dropout)
        ####lesnet8,9
        self.conv_down = nn.Conv2d(dim, dim, kernel_size=3, stride=8, padding=1)
        ####lesnet9 multi-pixelshuffle_block to save params
        # self.conv_up = nn.ConvTranspose2d(dim // 2, dim // 2, kernel_size=8, stride=8)
        self.conv_up = nn.Sequential(
                nn.Upsample(scale_factor=8),
    ...
    def forward(self, x):
        ...
        input_q = self.q2(input.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        input_q = self.conv_down(input_q)
        q1 = input_q.reshape(B, H * W // up_down_scale // up_down_scale, self.heads, C // self.heads).permute(0, 2,
                                                                                                                   1,
                                                                                                                   3)  # B,H*W,head//2,C//head
        x_ = input  # B, C, H, W
        x_1 = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)  # 降采样
        x_1 = self.act(self.norm(x_1))
        kv1 = self.kv1(x_1).reshape(B, -1, 2, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
        k1, v1 = kv1[0], kv1[1]  # B head N C

        attn1 = (q1 @ k1.transpose(-2, -1)) * self.scale  # B head Nq Nkv
        attn1 = attn1.softmax(dim=-1)
        attn1 = self.attn_drop(attn1)
        out1 = (attn1 @ v1).transpose(2, 3).reshape(B, C, H // up_down_scale, W // up_down_scale)
        out1 = self.conv_up(out1)
        out1 = rearrange(out1, 'b d (x w1) (y w2) -> b x y w1 w2 d', w1=window_size, w2=window_size)
        # output = torch.cat([out, out1], dim=5)
        output = self.to_output(out1)

        return output



class OSA_Block_hybrid_modified_s_global_spatial_SA(nn.Module):
    def __init__(self, channel_num=64, bias=True, ffn_bias=True, window_size=8, with_pe=False, dropout=0.0):
        super(OSA_Block_hybrid_modified_s_global_spatial_SA, self).__init__()
        ...
        self.layer = nn.Sequential(
            ...
            Rearrange('b d (x w1) (y w2) -> b x y w1 w2 d', w1=w, w2=w),  # 对输入进行维度重排 b:B,d:C,w:W,
            PreNormResidual(channel_num,
                            hybrid_Attention_GS_SA(dim=channel_num, dim_head=channel_num // 4, dropout=dropout,
                                                      window_size=window_size, with_pe=with_pe)),  # spatial-attention
            Rearrange('b x y w1 w2 d -> b d (x w1) (y w2)'),
            Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim=channel_num, dropout=dropout)),
            # FeedForward network
...
class OSA_Block_hybrid_modified_s_no_cnnbranch(nn.Module):
    def __init__(self, channel_num=64, bias=True, ffn_bias=True, window_size=8, with_pe=False, dropout=0.0):
        super(OSA_Block_hybrid_modified_s_no_cnnbranch, self).__init__()
