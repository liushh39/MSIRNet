...
class FeMaSRModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.LQ_stage:
            ...
            self.load_network(self.net_g, load_path, False)  ### load for LQ stage,but not strict_load because of X2,X4 encoder
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('perceptual_vgg_opt'):
            self.cri_perceptual_vgg = build_loss(train_opt['perceptual_vgg_opt']).to(self.device)
            self.model_to_device(self.cri_perceptual_vgg)
        else:
            self.cri_perceptual_vgg = None
    ...
    def feed_data(self, data):
        ...
        self.mask_grid1 = data['mask_grid1'].to(self.device)  # semantic_prior
        self.mask_grid2 = data['mask_grid2'].to(self.device)  # semantic_prior
        self.mask_grid3 = data['mask_grid3'].to(self.device)  # semantic_prior
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.LQ_stage:
            with torch.no_grad():
                self.gt_rec, _, _, gt_indices = self.net_hq(self.gt)  ##生成 codebook 标签gt_indices

            self.output, l_codebook, l_semantic, _ = self.net_g(self.lq, self.mask_grid1, self.mask_grid2, self.mask_grid3, gt_indices)
            ######## -----------------measure param-------------#############
            # test_images = torch.rand((1, 3, 256, 256)).cuda()
            # mask_grid111 = torch.rand((1, 64, 256, 256)).cuda()
            # mask_grid222 = torch.rand((1, 128, 256, 256)).cuda()
            # mask_grid333 = torch.rand((1, 256, 256, 256)).cuda()
            # macs, params = profile(self.net_g, inputs=(test_images, mask_grid111, mask_grid222, mask_grid333))
        ...
        # perceptual_vgg loss
        if self.cri_perceptual_vgg:
            l_percep_vgg, l_style_vgg = self.cri_perceptual_vgg(self.output, self.gt)
            if l_percep_vgg is not None:
                l_g_total += l_percep_vgg.mean()
                loss_dict['l_percep_vgg'] = l_percep_vgg.mean()
            if l_style_vgg is not None:
                l_g_total += l_style_vgg
                loss_dict['l_style_vgg'] = l_style_vgg
    ...
    def test(self):
        ...
        min_size = 8000 * 8000 # use smaller min_size with limited GPU memory 8000
        lq_input = self.lq
        _, _, h, w = lq_input.shape
        if h*w < min_size:
            self.output = net_g.test(lq_input, self.mask_grid1, self.mask_grid2, self.mask_grid3)
        else:
            self.output = net_g.test_tile(lq_input, self.mask_grid1, self.mask_grid2, self.mask_grid3)
        self.net_g.train()
